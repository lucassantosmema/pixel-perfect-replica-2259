# Mejoras Web Mareas Vivas — Plan de Implementación

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Añadir menú móvil hamburger (A), animaciones fade-in al hacer scroll (C) y refactorizar `index.tsx` separando datos y componentes (D).

**Architecture:** Refactor puro de `index.tsx` extrayendo datos a `src/data/`, componentes a `src/components/`. Nuevo `MobileMenu` con drawer lateral y overlay. Hook `useScrollReveal` con `IntersectionObserver` nativo. Sin librerías externas adicionales.
...
|--------|---------|-----------------|
| Crear | `src/data/menu-data.ts` | Array `menu` (la carta completa) |
| Crear | `src/data/slides-data.ts` | Arrays `heroSlides` y `experienceSlides` |
| Crear | `src/components/HeroCarousel.tsx` | Carrusel hero extraído |
| Crear | `src/components/ExperienceCarousel.tsx` | Carrusel experiencia extraído |
| Crear | `src/components/ReservationForm.tsx` | Formulario de reserva extraído |
| Crear | `src/components/MobileMenu.tsx` | Hamburger + drawer lateral (nuevo) |
| Crear | `src/hooks/use-scroll-reveal.ts` | IntersectionObserver para fade-in |
| Modificar | `src/routes/index.tsx` | Solo layout + imports (~120 líneas) |
| Modificar | `src/styles.css` | Clases `.reveal` y `.reveal.visible` |

---

### Task 1: Datos — menu-data.ts

**Files:**
- Create: `src/data/menu-data.ts`

- [ ] **Step 1: Crear el archivo con la carta completa**
...
```bash
cd pixel-perfect-replica-2259 && npx tsc --noEmit
```
...
```bash
git add src/data/menu-data.ts
git commit -m "refactor: extract menu data to src/data/menu-data.ts"
```

---

### Task 2: Datos — slides-data.ts

**Files:**
- Create: `src/data/slides-data.ts`

- [ ] **Step 1: Crear el archivo con los dos arrays de slides**
...
import catFlan from "@/assets/cat-flan.jpg";
import catCheesecake from "@/assets/cat-cheesecake.jpg";
...
export const heroSlides = [
  ...
  {
    img: catFlan,
    kicker: "Postres caseros",
    title: "Dulce final,",
    titleItalic: "hecho en casa.",
    desc: "Flan casero con caramelo artesanal. El broche perfecto para cada comida.",
  },
  {
    img: catCheesecake,
    kicker: "Repostería de autor",
    title: "Cheesecake,",
    titleItalic: "sin igual.",
    desc: "Base crujiente, crema sedosa. Cada porción, una pequeña obra.",
  },
];

export const experienceSlides = [
  {
    img: "https://images.unsplash.com/photo-1611329857570-f02f340e7378?w=1400&q=80",
    label: "Billar",
    title: "BILLAR",
    desc: "Mesas profesionales para partidas en buena compañía. El juego como excusa perfecta.",
    hours: "18:00 – 00:00",
  },
  {
    ...
    label: "Terraza",
    title: "TERRAZA",
    desc: "Brisa atlántica, vistas al mar y la mejor compañía. El lugar perfecto para el aperitivo.",
    hours: "11:00 – 00:00",
  },
  {
    ...
    label: "Restaurante",
    title: "RESTAURANTE",
    desc: "Almuerzos y cenas con nuestra carta completa y platos del día elaborados con producto de cercanía.",
    hours: "12:00 – 23:30",
  },
];
```
...
```bash
git add src/data/slides-data.ts
git commit -m "refactor: extract slides data to src/data/slides-data.ts"
```

---

### Task 3: Extraer HeroCarousel
...
git commit -m "refactor: extract HeroCarousel to src/components/"
```

---

### Task 4: Extraer ExperienceCarousel
...
git commit -m "refactor: extract ExperienceCarousel to src/components/"
```

---

### Task 5: Extraer ReservationForm

**Files:**
- Create: `src/components/ReservationForm.tsx`
...
git commit -m "refactor: extract ReservationForm to src/components/"
```

---

### Task 6: Crear MobileMenu
...
git commit -m "feat: add MobileMenu component with hamburger + drawer"
```

---

### Task 7: Scroll reveal — CSS + hook

**Files:**
- Modify: `src/styles.css`
- Create: `src/hooks/use-scroll-reveal.ts`

- [ ] **Step 1: Añadir clases reveal a styles.css**

Añadir al final del bloque `@layer base` en `src/styles.css`:
...
El bloque `@layer base { ... }` queda así al final:
...
```bash
git add src/styles.css src/hooks/use-scroll-reveal.ts
git commit -m "feat: add scroll reveal CSS + useScrollReveal hook"
```

---

### Task 8: Reescribir index.tsx

**Files:**
- Modify: `src/routes/index.tsx`

Reemplazar el contenido **completo** de `src/routes/index.tsx` con el siguiente código. Elimina los componentes y datos que ya viven en sus propios archivos e integra todo lo nuevo.

- [ ] **Step 1: Reemplazar index.tsx completo**
...
Arrancar el servidor de desarrollo y confirmar que:
- El sitio se ve idéntico en desktop
- En mobile aparece el hamburger junto al botón Reservar
- Al hacer scroll las secciones hacen fade-in suave
...
git commit -m "refactor: rewrite index.tsx using extracted components and data files"
```
...
- [ ] En viewport < 1024px: hamburger visible, drawer abre/cierra correctamente
- [ ] Secciones hacen fade-in al scrollear (no flash de contenido)
- [ ] Comportamiento visual idéntico al original en desktop
- [ ] Todos los links de navegación siguen funcionando
